...
-- 导出 onethink_trunk 的数据库结构
CREATE DATABASE IF NOT EXISTS `onethink_trunk` /*!40100 DEFAULT CHARACTER SET utf8 */;
USE `onethink_trunk`;


-- 导出  表 onethink_trunk.onethink_action 结构
...
-- 正在导出表  onethink_trunk.onethink_action 的数据：11 rows
...
-- 导出  表 onethink_trunk.onethink_action_log 结构
...
-- 正在导出表  onethink_trunk.onethink_action_log 的数据：13 rows
...
INSERT INTO `onethink_action_log` (`id`, `action_id`, `user_id`, `action_ip`, `model`, `record_id`, `remark`, `status`, `create_time`) VALUES
	...
	(11, 1, 1, 2130706433, 'member', 1, 'admin在2015-11-07 17:01登录了后台', 1, 1446886884),
	(12, 1, 1, 2130706433, 'member', 1, 'admin在2015-11-10 17:18登录了后台', 1, 1447147133),
	(13, 1, 1, 2130706433, 'member', 1, 'admin在2015-11-10 17:49登录了后台', 1, 1447148999);
/*!40000 ALTER TABLE `onethink_action_log` ENABLE KEYS */;


-- 导出  表 onethink_trunk.onethink_addons 结构
...
-- 正在导出表  onethink_trunk.onethink_addons 的数据：8 rows
...
-- 导出  表 onethink_trunk.onethink_attachment 结构
...
-- 正在导出表  onethink_trunk.onethink_attribute 的数据：32 rows
...
-- 导出  表 onethink_trunk.onethink_auth_extend 结构
...
-- 正在导出表  onethink_trunk.onethink_auth_extend 的数据：8 rows
...
-- 导出  表 onethink_trunk.onethink_auth_group 结构
...
-- 正在导出表  onethink_trunk.onethink_auth_group 的数据：2 rows
...
-- 导出  表 onethink_trunk.onethink_auth_group_access 结构
...
-- 正在导出表  onethink_trunk.onethink_auth_rule 的数据：213 rows
...
-- 导出  表 onethink_trunk.onethink_category 结构
...
-- 正在导出表  onethink_trunk.onethink_category 的数据：3 rows
...
-- 导出  表 onethink_trunk.onethink_channel 结构
...
-- 正在导出表  onethink_trunk.onethink_channel 的数据：3 rows
...
-- 导出  表 onethink_trunk.onethink_config 结构
...
-- 正在导出表  onethink_trunk.onethink_config 的数据：27 rows
...
-- 导出  表 onethink_trunk.onethink_demo 结构
...
-- 正在导出表  onethink_trunk.onethink_demo 的数据：17 rows
/*!40000 ALTER TABLE `onethink_demo` DISABLE KEYS */;
INSERT INTO `onethink_demo` (`id`, `title`, `description`, `content`, `create_time`, `uid`, `status`, `cover_id`, `deadline`) VALUES
	(1, '名称', '描述', '详情', 1446197327, 1, 0, 0, 0),
	(2, 'aaaa', 'aaaaaaaaaaa', 'aaaaaaaaaaaaaaaaaaaaa', 1446619556, 1, 1, 3, 0),
	(4, '模型名称1', '模型描述', '<span style="color:#404040;font-family:\'microsoft yahei\', Helvetica, Tahoma, Arial, sans-serif;font-size:14px;font-weight:bold;line-height:30px;background-color:#F6F6F6;">模型详情</span>', 1447147366, 1, 1, 0, 1447201980),
	(5, '模型名称2', '模型描述2', '<span style="color:#404040;font-family:\'microsoft yahei\', Helvetica, Tahoma, Arial, sans-serif;font-size:14px;font-weight:bold;line-height:30px;background-color:#F6F6F6;">模型详情2</span>', 1447147534, 1, 1, 0, 1447493100),
	(9, '', '', '', 0, 1, 0, 0, 0),
	(10, '', '', '', 0, 1, 0, 0, 0),
	(11, 'aaaaaaaaa', 'bbbbbbbbbbb', 'cccccccccccccc', 1447149225, 1, 1, 0, 1447321740),
	(12, 'eeeeeee', 'eeeeeeeee', 'eeeeeeeeeeeee', 1447149244, 1, 1, 0, 0),
	(13, '', '', '', 0, 1, 0, 0, 0),
	(14, '', '', '', 0, 1, 0, 0, 0),
	(15, '', '', '', 0, 1, 0, 0, 0),
	(16, '', '', '', 0, 1, 0, 0, 0),
	(17, '', '', '', 0, 1, 0, 0, 0),
	(18, '', '', '', 1447149800, 1, 1, 0, 0),
	(19, '', '', '', 0, 1, 0, 0, 0),
	(20, '1', '', '', 1447149939, 0, 1, 0, 0),
	(21, '', '', '', 1447150439, 0, 1, 0, 0);
/*!40000 ALTER TABLE `onethink_demo` ENABLE KEYS */;


-- 导出  表 onethink_trunk.onethink_document 结构
...
) ENGINE=MyISAM AUTO_INCREMENT=23 DEFAULT CHARSET=utf8 COMMENT='文档模型基础表';

-- 正在导出表  onethink_trunk.onethink_document 的数据：22 rows
...
-- 导出  表 onethink_trunk.onethink_document_article 结构
...
-- 正在导出表  onethink_trunk.onethink_document_article 的数据：17 rows
...
-- 导出  表 onethink_trunk.onethink_document_download 结构
...
-- 正在导出表  onethink_trunk.onethink_document_download 的数据：5 rows
...
-- 导出  表 onethink_trunk.onethink_file 结构
...
-- 正在导出表  onethink_trunk.onethink_file 的数据：5 rows
...
-- 导出  表 onethink_trunk.onethink_hooks 结构
...
-- 正在导出表  onethink_trunk.onethink_hooks 的数据：12 rows
...
-- 导出  表 onethink_trunk.onethink_member 结构
...
-- 正在导出表  onethink_trunk.onethink_member 的数据：1 rows
...
INSERT INTO `onethink_member` (`uid`, `nickname`, `sex`, `birthday`, `qq`, `score`, `login`, `reg_ip`, `reg_time`, `last_login_ip`, `last_login_time`, `status`) VALUES
	(1, 'admin', 0, '0000-00-00', '', 50, 10, 0, 1445657400, 2130706433, 1447148999, 1);
/*!40000 ALTER TABLE `onethink_member` ENABLE KEYS */;


-- 导出  表 onethink_trunk.onethink_menu 结构
...
-- 正在导出表  onethink_trunk.onethink_menu 的数据：112 rows
...
-- 导出  表 onethink_trunk.onethink_message 结构
...
-- 正在导出表  onethink_trunk.onethink_model 的数据：3 rows
...
-- 导出  表 onethink_trunk.onethink_picture 结构
...
-- 正在导出表  onethink_trunk.onethink_picture 的数据：3 rows
...
-- 导出  表 onethink_trunk.onethink_ucenter_admin 结构
...
-- 正在导出表  onethink_trunk.onethink_ucenter_member 的数据：1 rows
...
INSERT INTO `onethink_ucenter_member` (`id`, `username`, `password`, `email`, `mobile`, `reg_time`, `reg_ip`, `last_login_time`, `last_login_ip`, `update_time`, `status`) VALUES
	(1, 'admin', 'a9b4ad048c33807db124a359ddcaf534', '492948155@qq.com', '', 1445657400, 2130706433, 1447148999, 2130706433, 1445657400, 1);
/*!40000 ALTER TABLE `onethink_ucenter_member` ENABLE KEYS */;


-- 导出  表 onethink_trunk.onethink_ucenter_setting 结构
...
-- 正在导出表  onethink_trunk.onethink_userdata 的数据：0 rows
